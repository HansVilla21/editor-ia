...
> 3. **El guion**, si en la grabación repetiste frases hasta que salieran bien: una frase por línea. Con eso elijo la mejor toma de cada una. *(Si no lo tenés, lo armo yo con lo que dijiste y te lo muestro.)*
...
- **El cierre:** si pide que comenten una palabra, `CTA.palabra` es esa palabra y `CTA.recibe` es lo que dice en la grabación que va a mandar; si no lo dice, preguntalo una vez, porque no se puede prometer algo que no existe, y si tampoco contesta, `recibe` queda vacío. Si no pide comentar nada, el cierre es su última frase repartida en las tres líneas (`pide`, `palabra`, `recibe`), sin agregar ninguna promesa.
...
| Fase 2, "El final": mira a otro lado antes de 0,4 s después de la última palabra, y "Final" no dice qué hacer | "Cuando terminás de hablar mirás a otro lado enseguida. ¿Termino igual un instante después (con el cierre en pantalla casi no se nota), corto justo ahí (queda abrupto), o cierro con una pantalla final con el texto del cierre?" | Termina a los 0,4 s |
...
Aparte de esto sigue en pie lo que pide la skill: el OK antes de bajar cualquier cosa de una fuente nueva (efectos nuevos, logos, imágenes).
...
- **Dos o tres portadas**, salvo que "Portada" diga que elige Claude: la elegida es `portada.png` y las otras, con los mismos textos sobre otro cuadro, van en `versiones/portada-b.png` y `versiones/portada-c.png`.
...
**Las versiones siguientes del mismo video** (v2, v3…) se entregan con dos líneas: qué cambió y "¿Así queda, o algo más?". `video-final.mp4` pasa a ser la versión nueva; los renders anteriores siguen en `versiones/`.

**Anotá, una sola vez por video** (en la primera entrega, no en cada versión):

- `videosHechos` más uno en `estado.json`.
- En `memory/decisiones.md`: una línea en "Videos" (que se completa si después hay versiones), la música usada y la opción B en "Música", y si la grabación era cruda, los umbrales que funcionaron en "Umbrales de corte".
...
2. **Decidí si es de una vez o para siempre.**
   ...
   - **Para siempre:** es un gusto o una falla que se puede repetir: "la *s* de *frases* quedó cortada", "los subtítulos se leen chicos", "no me gusta la música electrónica".
   - Si no está claro, preguntá en una línea: "¿Esto es solo para este video o para todos los que vienen?" Por defecto: si es una falla (algo sonó o se vio mal), para siempre; si es un gusto, solo este video, y si lo vuelve a pedir en otro, pasa a ser para siempre.
